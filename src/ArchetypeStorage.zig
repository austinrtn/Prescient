...
const Component = Registry.Component.Enum;
...
pub fn Archetype(comptime components: []const Component) type {
    ...
    const EntTypeSlices = Registry.Component.GetTypeOfComponents(components, true);
    const bit_mask = Registry.Component.getBitmaskOfComponents(components);
...
fn compsToArrayList(comptime components: []const Component) type {
    ...
    for(components, 0..) |comp, i| {
        const T = Registry.Component.GetTypeByField(comp);
