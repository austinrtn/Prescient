...
New [[1.00 - Entity Pools|Entity Pools]] can be added to the top of [[1.07 - Pool Registry|Pool Registry]].  Pools must be made public in order to be registered correctly.  Prescient's build system will automatically update the registry once it is added to the file underneath the [[1.04 - General Pool|General Pool]] which is the "default" pool in Prescient.  Developers can configure the pool by passing a [[1.02 - PoolConfig|PoolConfig]] struct as the Entity Pool's parameter.

>[!example]
>```zig
>// project/src/registries/PoolRegistry.zig
...
Once the pool is properly added to the registry (as depicted above), Prescient's [[prebuild]] system will register the pool completely, and will be ready for use.
# Good Pool Design 
