...
# How To Use
You can select which storage strategy you want to use when defining entity pools within the [[1.15 - Pool Registry|Pool Registry]] within the [[1.02 - PoolConfig|PoolConfig]] struct.  Pass a `StorageStrategy.StorageStrategy` enum in the `storage_strategy` argument of PoolConfig.  Values of `StorageStrategy` are Sparse and Archetype. 
...
##### Next:  [[1.05 - Storage Strategy (Architecture)|Storage Strategy (Architecture)]]
# See Also
